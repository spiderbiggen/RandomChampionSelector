...
public class DDragon {
    ...
    private static final String DEFAULT_VERSION = "8.4.1";
    ...
    public Disposable verifyImages(List<Champion> champions, ProgressCallback callback,
                                   Consumer<List<ImageDescriptor>> consumer, @NonNull Consumer<Throwable> onError) {
        ...
        return Observable.fromIterable(newImages)
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .doOnNext(p -> callback
                .onProgressUpdate(VERIFY_SUCCESS, downloadCount.incrementAndGet(), total))
    ...
    public Disposable downloadAllImages(List<ImageDescriptor> champions, @NonNull ProgressCallback callback,
                                        @NonNull Action onComplete, @NonNull Consumer<Throwable> onError) {
        final AtomicInteger downloadCount = new AtomicInteger();
        ...
        return Observable.fromIterable(champions)
            ...
            .flatMap(item -> Observable.just(item)
                .subscribeOn(Schedulers.io())
                .map(imDesc -> imDesc
                    .verifyDownload(this, compressFormat, quality)), MAX_CONCURRENCY)
    ...
    @NonNull
    private File getChampionFile(@NonNull String champion, @NonNull ImageType type,
                                 @NonNull Bitmap.CompressFormat format) throws
        IOException {
        FileStorage storage = FileStorage.getInstance();
        return new File(storage.getChampionImageDir(), String
            .format("%s_%s.%s", champion, type.name().toLowerCase(), format.name()
                .toLowerCase()));
    }

    public Bitmap getChampionBitmap(@NonNull Champion champion, @NonNull ImageType type) throws
        IOException {
    ...
    protected void saveBitmap(@NonNull final File file, final Bitmap bitmap, Bitmap.CompressFormat compressFormat,
                              int quality) throws
        IOException {
    ...
    private DDragonService createService() {
        Retrofit retrofit = new Retrofit.Builder()
            .baseUrl(BASE_URL)
            .addConverterFactory(getChampionsDeserializer())
            .addCallAdapterFactory(RxJava2CallAdapterFactory.createAsync())
            .build();
        return retrofit.create(DDragonService.class);
    }

    private Converter.Factory getChampionsDeserializer() {
        return new JsonConverterFactory();
    }

    private int getQuality() {
        return preferences.getInt("pref_image_quality", resources
            .getInteger(R.integer.pref_image_quality_default));
    }

    private String getLocal() {
        return preferences
            .getString("pref_language", resources.getString(R.string.pref_language_default));
    }

    private Bitmap.CompressFormat getCompressionMethod() {
        return Bitmap.CompressFormat.valueOf(preferences.getString("pref_image_type", resources
            .getString(R.string.pref_title_image_type_default)));
    }
