...
public class DatabaseManager implements IDataInteractor {
    ...
    public void useContext(Context context) {
        database = Room.databaseBuilder(context, SimpleDatabase.class, "random_champion_main")
    ...
    private Flowable<List<Champion>> getChampionListFlowable(String role) {
        return isWildcardRole(role)
            ? database.championDAO().getAll()
            : database.championDAO().getAll(role);
    }

    private Flowable<Champion> getRandomChampionSingle(String role) {
        return isWildcardRole(role)
            ? database.championDAO().getRandom()
            : database.championDAO().getRandom(role);
    }
