...
class CustomConverter : Converter.Factory() {
    ...
    private class ChampionListConverter : Converter<ResponseBody, List<ApiChampion>> {

        @Throws(IOException::class)
        override fun convert(responseBody: ResponseBody): List<ApiChampion> {
            try {
                responseBody.use {
                    val string = it.string()
                    val jsonObject = JSONObject(string).getJSONObject("data")
                    val list = mutableListOf<ApiChampion>()
                    val stringIterator = jsonObject.keys()
                    while (stringIterator.hasNext()) {
                        val key = stringIterator.next()
                        try {
                            val champion = ApiChampion.parse(jsonObject.getJSONObject(key))
                            list.add(champion)
    ...
    companion object {
        ...
        private val LIST_CHAMPION = object : ParameterizedType {
            ...
            override fun getActualTypeArguments(): Array<Type> {
                return arrayOf(ApiChampion::class.java)
            }
        }
